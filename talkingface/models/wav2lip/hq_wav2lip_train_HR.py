...
from prefetch_generator import BackgroundGenerator  # 数据预加载
...
def train(device, model, disc, train_data_loader, test_data_loader, optimizer, disc_optimizer,
          lr_scheduler=None, disc_lr_scheduler=None,
    ...
    while global_epoch < nepochs:
        ...
        for step, (x, indiv_mels, mel, gt) in prog_bar:
            ...
            if disc_lr_scheduler is not None:
                # lr_scheduler.step(global_step) # is ok?
                if resumed_step > 0:  # resume
                    disc_lr_scheduler.step(global_step)
                else:
            ...
            if global_step == 1 or global_step % hparams.eval_interval == 0:
                with torch.no_grad():
                    ...
                    if average_sync_loss <= best_loss:
                        ...
                        save_checkpoint(
                            disc, disc_optimizer, global_step, checkpoint_dir, global_epoch, prefix='disc_',
                            is_best=True)
                    print("best_loss: ", best_loss)
                    print("best_step: ", best_step)
            prog_bar.set_description(
                'L1: {}, Sync: {}, Percep: {} | Fake: {}, Real: {}, lr: {}'.format(running_l1_loss / (step + 1),
...
def save_checkpoint(model, optimizer, step, checkpoint_dir, epoch, prefix='', is_best=False):
    if is_best:
        checkpoint_path = join(
            checkpoint_dir, "{}checkpoint_best.pth".format(prefix))
    else:
        checkpoint_path = join(
...
if __name__ == "__main__":
    ...
    # disc_lr_scheduler = WarmupPolyLR(disc_optimizer, warmup_iters=500, max_iters=20000000)
    lr_scheduler = None
